...
K = ROOT / "kernel"
KO = E / "stock-custom-ldo-wl2868.ko"
...
export_names = {"will_ldo_en", "will_ldo_vout"}
with (K / "phase4-custom-ldo-wl2868-functions.tsv").open("w", newline="") as f:
...
with (K / "phase4-custom-ldo-wl2868-objects.tsv").open("w", newline="") as f:
    w = csv.writer(f, delimiter="\t", lineterminator="\n")
...
with (K / "phase4-custom-ldo-wl2868-relocations.tsv").open("w", newline="") as f:
    w = csv.writer(f, delimiter="\t", lineterminator="\n")
...
with (K / "phase4-custom-ldo-wl2868-imports.tsv").open("w", newline="") as f:
    w = csv.writer(f, delimiter="\t", lineterminator="\n")
...
with (K / "phase4-custom-ldo-wl2868-modversions.tsv").open("w", newline="") as f:
    w = csv.writer(f, delimiter="\t", lineterminator="\n")
...
with (K / "phase4-custom-ldo-wl2868-exports.tsv").open("w", newline="") as f:
    w = csv.writer(f, delimiter="\t", lineterminator="\n")
    ...
    for s in funcs:
        if s["name"] in export_names:
            w.writerow([s["name"], s["section"], f"0x{s['value']:x}", s["size"], s["bind"], kcfi_id(s),
                        f"0x{export_crc[s['name']]:08x}", "", "custom_ldo"])

# String inventory for both string-bearing sections. Add relocation owners for .rodata.str1.1.
str_data = (E / "stock-rodata-str1-1.bin").read_bytes()
str_xrefs = {}
for r in relocs:
    if r["symbol"] == ".rodata.str1.1":
        try:
            add = int(r["addend"], 16)
        except ValueError:
            continue
        str_xrefs.setdefault(add, []).append(owner_for(r["relocation_section"], r["offset"]))
strings = []
i = 0
while i < len(str_data):
    j = str_data.find(b"\0", i)
    if j < 0:
        j = len(str_data)
    if j > i:
        b = str_data[i:j]
        # Keep printk control prefixes and escape control bytes/newlines so one
        # physical TSV row always represents one exact NUL-terminated string.
        text = "".join(chr(x) if 32 <= x < 127 and x != 92 else f"\\x{x:02x}" for x in b)
        strings.append((i, text))
    i = j + 1
with (K / "phase4-custom-ldo-wl2868-strings.tsv").open("w", newline="") as f:
    w = csv.writer(f, delimiter="\t", lineterminator="\n")
    w.writerow(["section", "offset_hex", "string", "relocation_xref_count", "owners"])
    for off, text in strings:
        owners = sorted(set(x for x in str_xrefs.get(off, []) if x))
        w.writerow([".rodata.str1.1", f"0x{off:x}", text, len(str_xrefs.get(off, [])), ",".join(owners)])
