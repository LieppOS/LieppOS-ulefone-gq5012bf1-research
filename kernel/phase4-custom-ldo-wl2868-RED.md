...
The donor has compatible `willsemi,wl2868c`; stock has `will,wl2864c_pmu`. The donor has no `will_ldo_vout` or `will_ldo_en` exports, no `memdup_user` path, and no stock optional-`vin1` then `reset` probe sequence (the stock error log calls `vin1` `vin1_en`). Its voltage tables are useful only as family context and were not copied as proof of stock semantics.
