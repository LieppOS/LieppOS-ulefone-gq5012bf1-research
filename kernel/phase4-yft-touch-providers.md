...
This is therefore a shared YFT touchscreen/device-information provider rather
than an FT3680-only helper.
...
2. yft_devinfo.ko
3. rebuild FT3680 against the reconstructed providers
4. continue the remaining FT3680-specific reconstruction.

Note for step 2: the four `yft_devinfo` byte-array exports
(`touch_fw_version`, `second_touch_fw_version`, `tinylcd_fw_version`,
`fuelgauge_fw_version`) have already been type-resolved as `char NAME[30]` —
all four stock CRCs (`0xd0815107`, `0x7198d58d`, `0x36887f32`, `0x8c280260`)
reproduce exactly from that declaration using the genksyms CRC model in
`$GKI_WS/lieppos/yft-tpd-gesture-recon/tools/yft-crc-search.py`.
...
Relevant exported ABI includes:
