 ...
 *
 * setup_readproc_group() is intentionally placed before
 * the existing writer helper definitions, so declare the
...
extern slong early_setgroups_rc;

static slong xsetgroups(
    ulong count,
...
/*
 * Android procfs is mounted:
 *
 *   hidepid=invisible,gid=3009
 *
 * AID_READPROC = 3009.
 *
 * This changes supplementary groups ONLY for the
 * diagnostic watchdog child. PID 1 and Android
 * services are not modified.
 */
static void setup_readproc_group(
    struct writer *w
)
{
...
static void record_early_readproc_state(
    struct writer *w
)
{
    writer_str(
        w,
        "\n=== EARLY_READPROC_SETUP ===\n"
    );
...
void capture_main(void)
{
    ...
    /*
     * V10 uses the exact physical pstore block device.
     */
    fd = xopen(
    ...
    /*
     * Write and flush immediately so even a later
     * blocking probe leaves V10 evidence behind.
     */
    writer_str(
        &w,
        "ULEFONE_DIAG_V10\n"
        "DEVICE=Armor_29_Pro_Thermal\n"
        "CAPTURE_DEVICE=/dev/block/by-name/pstore\n"
        "SCRATCH_OFFSET=0x08100000\n"
        "PURPOSE=minimal-phase-flushed-readproc\n"
        "PHASE=HEADER_WRITTEN\n"
    );

    xfsync(fd);

    record_early_readproc_state(
        &w
    );

    writer_str(
        &w,
        "PHASE=EARLY_READPROC_RECORDED\n"
    );
