...
def main() -> int:
    ...
    parser.add_argument("--aosp-replaced", type=Path, default=None,
                        help="stock paths owned by AOSP/ROM modules "
                             "(default: aosp-replaced-files.txt next to this "
                             "research checkout)")
    parser.add_argument("--recovery-tree", type=Path, default=None,
                        help="OrangeFox recovery overlay root; defaults to the "
                             "device tree, which only carries recovery-only "
                             "files while the overlay is applied")
    ...
    aosp_replaced = proprietary_entries(aosp_replaced_path)
    checks: list[Check] = []
    ...
    # Recovery-only assets live in the OrangeFox overlay repository. They are
    # audited when the overlay is applied to the device tree, or when
    # --recovery-tree points at the overlay checkout.
    recovery_board = board + text(recovery_tree / "BoardConfigFox.mk")
    security_rc = text(recovery_tree / "recovery/root/init.recovery.gq5012bf1.security.rc")
    boot_rc = text(recovery_tree / "recovery/root/init.recovery.mt6878.rc")
    recovery_available = bool(security_rc and boot_rc)
    ...
    check("trustkernel-model", "PRODUCT_MODEL := Armor 29 Pro" in "\n".join(text(path) for path in product_files + [recovery_tree / "twrp_gq5012bf1.mk"]), "security-facing product model is Armor 29 Pro")
    if recovery_available:
        ...
        check("usb-configfs", "TW_EXCLUDE_DEFAULT_USB_INIT := true" in recovery_board and "11201000.usb0" in boot_rc, "recovery configfs USB invariant retained")
    else:
        checks.append(Check("recovery-overlay", "WARN", "OrangeFox recovery overlay not present; recovery-only invariants not audited (pass --recovery-tree)"))
    ...
    if args.stock_partitions:
        ...
        closure = subprocess.run(
            [os.sys.executable, str(here / "elf_closure.py"),
             "--device", str(device),
             "--aosp-replaced", str(aosp_replaced_path),
