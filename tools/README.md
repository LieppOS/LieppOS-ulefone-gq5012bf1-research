...
python3 tools/inventory_device.py \
  --stock-root workspace/gq5012bf1/stock/partitions \
  --stock-snapshot workspace/gq5012bf1/snapshots/stock \
  --recovery-snapshot workspace/gq5012bf1/snapshots/recovery \
  --vendor-boot /path/to/unpacked/vendor_boot/platform_ramdisk \
  --out workspace/gq5012bf1/reports/inventory
```
...
python3 tools/generate_proprietary_candidates.py \
  ...
  --out workspace/gq5012bf1/reports/proprietary-files.review.txt
```
...
python3 tools/elf_closure.py \
  --device $DEVICE_TREE \
  --stock workspace/gq5012bf1/stock/partitions \
  --aosp-source "$ANDROID_BUILD_TOP" \
  --json workspace/gq5012bf1/reports/elf-closure.json \
  --check
```
...
python3 tools/snapshot_trust.py <snapshot-dir> \
  --stock-root workspace/gq5012bf1/stock/partitions --check
```
